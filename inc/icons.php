 ...
 */
function napoli_get_svg( $icon = null ) {
 ...
 */
function napoli_dropdown_icon_to_menu_link( $title, $item, $args, $depth ) {
	if ( 'primary' === $args->theme_location || 'secondary' === $args->theme_location ) {
		foreach ( $item->classes as $value ) {
			if ( 'menu-item-has-children' === $value || 'page_item_has_children' === $value ) {
				$title = $title . napoli_get_svg( 'expand' );
			}
		}
	}
...
add_filter( 'nav_menu_item_title', 'napoli_dropdown_icon_to_menu_link', 10, 4 );
 ...
 * @return string $svg SVG markup.
 */
function napoli_get_social_svg( $icon = null ) {
	// Return early if no icon was defined.
	if ( empty( $icon ) ) {
		return;
	}
...
add_filter( 'walker_nav_menu_start_el', 'napoli_social_icons_menu', 10, 4 );
 ...
 */
function napoli_supported_social_icons() {
