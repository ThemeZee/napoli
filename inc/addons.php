...
function napoli_theme_addons_scripts() {
	...
	if ( is_active_widget( 'TZWB_Facebook_Likebox_Widget', false, 'tzwb-facebook-likebox' )
		...
		wp_enqueue_style( 'napoli-widget-bundle', get_template_directory_uri() . '/assets/css/themezee-widget-bundle.css', array(), '20160421' );
	...
	if ( is_singular( 'post' ) ) {
		...
		wp_enqueue_style( 'napoli-related-posts', get_template_directory_uri() . '/assets/css/themezee-related-posts.css', array(), '20160421' );
 ...
 */
function napoli_amp_menu_toggle() {
	if ( napoli_is_amp() ) {
 ...
 */
function napoli_amp_menu_is_toggled() {
	if ( napoli_is_amp() ) {
		echo "[class]=\"'primary-navigation' + ( primaryMenuExpanded ? ' toggled-on' : '' )\"";
	}
}
 ...
 */
function napoli_amp_menu_dropdown_toggles( $item_output, $item, $depth, $args ) {
	...
	*/
	...
	// Add SVG icon.
	$dropdown_button .= $expanded ? napoli_get_svg( 'collapse' ) : napoli_get_svg( 'expand' );
	...
	$dropdown_button .= sprintf(
		...
		esc_attr( sprintf( "$expanded_state_id ? %s : %s", wp_json_encode( esc_html__( 'Collapse child menu', 'napoli' ) ), wp_json_encode( esc_html__( 'Expand child menu', 'napoli' ) ) ) ),
		esc_html( $expanded ? esc_html__( 'Collapse child menu', 'napoli' ) : esc_html__( 'Expand child menu', 'napoli' ) )
	);
...
add_filter( 'walker_nav_menu_start_el', 'napoli_amp_menu_dropdown_toggles', 10, 4 );
