 ...
 */
class Napoli_Magazine_Posts_Columns_Widget extends WP_Widget {
	...
	function __construct() {
		...
		// Setup Widget.
		parent::__construct(
			'napoli-magazine-posts-columns', // ID.
			sprintf( esc_html__( 'Magazine Posts: 2 Columns (%s)', 'napoli' ), wp_get_theme()->Name ), // Name.
			array(
				'classname' => 'napoli-magazine-posts-columns',
				'description' => esc_html__( 'Displays your posts from two selected categories. Please use this widget ONLY in the Magazine Homepage widget area.', 'napoli' ),
	...
	function widget( $args, $instance ) {
		...
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'widget_napoli_magazine_posts_columns', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_napoli_magazine_posts_columns', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	...
	function category_title( $args, $settings, $category_id, $category_title ) {
		...
		if ( ! empty( $widget_title ) ) :
			...
			if ( $category_id > 0 ) :
				...
				$link_title = sprintf( esc_html__( 'View all posts from category %s', 'napoli' ), get_cat_name( $category_id ) );
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'category_one_title' ); ?>"><?php esc_html_e( 'Left Category Title:', 'napoli' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'category_one' ); ?>"><?php esc_html_e( 'Left Category:', 'napoli' ); ?></label><br/>
			...
			?>
				...
				<p>
			<label for="<?php echo $this->get_field_id( 'category_two_title' ); ?>"><?php esc_html_e( 'Right Category Title:', 'napoli' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'category_two' ); ?>"><?php esc_html_e( 'Right Category:', 'napoli' ); ?></label><br/>
			<?php // Display Category One Select.
				$args = array(
					'show_option_all'    => esc_html__( 'All Categories', 'napoli' ),
					'show_count' 		 => true,
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'highlight_post' ); ?>">
				...
				<?php esc_html_e( 'Highlight first post (big image + excerpt)', 'napoli' ); ?>
			</label>
		</p>
...
<?php
	...
	public function delete_widget_cache() {
		...
		wp_cache_delete( 'widget_napoli_magazine_posts_columns', 'widget' );
 ...
 * Register Widget
 */
function napoli_register_magazine_posts_columns_widget() {

	register_widget( 'Napoli_Magazine_Posts_Columns_Widget' );

}
add_action( 'widgets_init', 'napoli_register_magazine_posts_columns_widget' );
